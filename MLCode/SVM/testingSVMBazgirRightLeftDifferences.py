...
def main():
    ...
    # take Right-Left differences
    # 0 = AF4-AF3 (13-0), 1 = F8-F7 (12-1), 2 = F4-F3 (11-2), 3 = FC6-FC5 (10-3), 4  = T8-T7 (9-4), 5 = P8-P7 (8-5), 6 = O2-O1 (7-6)
    channelPowersDifferences = {0:[],1:[],2:[],3:[],4:[],5:[],6:[]}
    channelPowersLabels = {0:channelLabels[0],1:channelLabels[1],2:channelLabels[2],3:channelLabels[3],4:channelLabels[4],5:channelLabels[5],6:channelLabels[6]}
    for trial in range(1280):
        channelPowersDifferences[0].append([channelPowers[13][trial][x]-channelPowers[0][trial][x] for x in range(len(channelPowers[13][trial]))])
        channelPowersDifferences[1].append([channelPowers[12][trial][x]-channelPowers[1][trial][x] for x in range(len(channelPowers[12][trial]))])
        channelPowersDifferences[2].append([channelPowers[11][trial][x]-channelPowers[2][trial][x] for x in range(len(channelPowers[11][trial]))])
        channelPowersDifferences[3].append([channelPowers[10][trial][x]-channelPowers[3][trial][x] for x in range(len(channelPowers[10][trial]))])
        channelPowersDifferences[4].append([channelPowers[9][trial][x]-channelPowers[4][trial][x] for x in range(len(channelPowers[9][trial]))])
        channelPowersDifferences[5].append([channelPowers[8][trial][x]-channelPowers[5][trial][x] for x in range(len(channelPowers[8][trial]))])
        channelPowersDifferences[6].append([channelPowers[7][trial][x]-channelPowers[6][trial][x] for x in range(len(channelPowers[7][trial]))])
    ...
    arousalY = {0:[], 1:[], 2:[], 3:[], 4:[], 5:[], 6:[]}
    valenceY = {0:[], 1:[], 2:[], 3:[], 4:[], 5:[], 6:[]}
    for channelInd in range(7):
