...
def main():
    ...
    for subjectNumber in range(1,33):
        ...
        for trialNumber in range(40):
            ...
            #add labels (+- 1 ) to corresponding Y lists depending if given label is > or < 4.5 (midpoint of rating)
            if trialLabel[0] > 4.5:
                valenceY.append(1)
            else:
                valenceY.append(-1)
    ...
    arousalXTrain, arousalXTest, arousalYTrain, arousalYTest = train_test_split(arousalX, arousalY, test_size = 0.3, random_state = 109)
    valenceXTrain, valenceXTest, valenceYTrain, valenceYTest = train_test_split(valenceX, valenceY, test_size = 0.3, random_state = 109)
    #train each SVM
