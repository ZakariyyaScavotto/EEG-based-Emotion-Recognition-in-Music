...
def main():
   ...
   #split data into test/train sets for each DBN
    ...
    # putting training data into test
    arousalXTest.insert(0, arousalXTrain[0])
    arousalYTest.insert(0, arousalYTrain[0])
    valenceXTest.insert(0, valenceXTrain[0])
    valenceYTest.insert(0, valenceYTrain[0])
   ...
   #check accuracy, using mean squared error now that it's a regression
    arousalPredicts = arousalClassifier.predict(arousalXTrain)
    valencePredicts = valenceClassifier.predict(valenceXTrain)
