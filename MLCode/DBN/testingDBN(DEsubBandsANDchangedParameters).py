...
def main():
   ...
   #duplicate to train on valence, arousal
   ...
   #split data into test/train sets for each DBN
    arousalXTrain, arousalXTest, arousalYTrain, arousalYTest = train_test_split(arousalX, arousalY, test_size = 0.2, random_state=42)
    valenceXTrain, valenceXTest, valenceYTrain, valenceYTest = train_test_split(valenceX, valenceY, test_size = 0.2, random_state=42)
   ...
   #creates/trains classifiers (since this is a DBN can now perform a regression task on continuous data)
    # arousalClassifier = SupervisedDBNRegression(hidden_layers_structure = [160,100,30,1],learning_rate_rbm=0.05,
    #                                         learning_rate=0.05, n_epochs_rbm=10, n_iter_backprop=200,
    #                                         batch_size=100, activation_function='relu')
    # valenceClassifier = SupervisedDBNRegression(hidden_layers_structure = [160,100,30,1],learning_rate_rbm=0.05,
    #                                         learning_rate=0.05, n_epochs_rbm=10, n_iter_backprop=200,
    #                                         batch_size=100, activation_function='relu')

    # TRYING THEIR PARAMETERS
    arousalClassifier = SupervisedDBNRegression(hidden_layers_structure=[100],learning_rate_rbm=0.01,learning_rate=0.01, 
                                                n_epochs_rbm=20, n_iter_backprop=200,batch_size=16,activation_function='relu')
    valenceClassifier = SupervisedDBNRegression(hidden_layers_structure=[100],learning_rate_rbm=0.01,learning_rate=0.01,
                                                n_epochs_rbm=20,n_iter_backprop=200,batch_size=16, activation_function='relu')
   ...
   #check accuracy
    ...
    print('Arousal r^2: ', metrics.r2_score(arousalYTest, arousalPredicts))
    print('Valence Mean Squared Error: ',metrics.mean_squared_error(valenceYTest, valencePredicts))
    print('Valence r^2: ', metrics.r2_score(valenceYTest, valencePredicts))
