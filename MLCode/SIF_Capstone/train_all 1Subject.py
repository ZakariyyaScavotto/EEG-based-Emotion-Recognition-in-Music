...
parser.add_argument('--config', default = 'config/training_config_Deep_isith.ini', type=str, help='an integer for the accumulator')
...
dir = 'grasp-and-lift-eeg-detection/'#config['data']['directory']
subject_num = 2#int(config['data']['subject #'])
kernel_size = 2000#int(config['training']['kernel_size'])# sliding window size to use
step = 1000#int(config['training']['step']) #  --the step between each slice. means overlap between batches is 1- step 
modelName = 'Deep_isith'#config['training']['model']
# num of epochs to train
nepochs = 60#int(config['training']['nepochs'])
loss_func =  torch.nn.CrossEntropyLoss()
batch_size = 128#int(config['training']['batch_size']) # batch_size is a hyper parameter to tune 
lr = 0.001#float(config['training']['lr'])


train_dir = dir + 'train1Subject/'
# val_dir = dir + 'validation/'
val_dir = dir + 'train1Subject/'
...
for j in range(1,13): # out loop train all subjects
    ...
    for i in range(1,7): # There are six events 1 - 6
        ...
        perf = []
        perf = train_model(model, ttype, train_loader, val_loader, optimizer, loss_func, epochs=nepochs)
        results.append(perf)
